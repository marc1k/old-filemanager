...
/// A flattened matrix of `Cell`s with a known `Region`.
pub struct Buffer {
    pub region: Region,
...
impl Buffer {
    ...
    /// Constructs a default `Buffer` of blank `Cell`s.
    pub fn blank(region: Region) -> Self {
        Buffer::filled(region, Cell::default())
    }

    /// Returns an `Iterator` of a `Cell` bound to a `Position` in the content.
